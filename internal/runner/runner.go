...
// ContextLabel formats a context size as a human-readable label.
...
// Runner orchestrates the benchmark execution lifecycle.
type Runner struct {
...
// New creates a Runner from configuration, detecting llama.cpp features.
...
// Plan generates a benchmark plan from configuration and existing results.
...
func (r *Runner) recordStartupFailure(
	group []domain.PlannedRun,
...
// Run executes the benchmark plan and reports progress via the TUI.
func (r *Runner) Run(ctx context.Context) error {
	...
	initial := viewmodel.BenchmarkTUIState{
		...
		BuildInfo:     viewmodel.BuildInfoView{CommitShort: or(r.Features.LlamaCpp.CommitShort, "unknown"), Branch: or(r.Features.LlamaCpp.Branch, "unknown"), Backend: or(r.Features.Backend, "server")},
		ModelName:     r.Config.ModelHF(),
		Progress:      viewmodel.ProgressState{ServersTotal: len(bench.RunnableGroups(plan)), JobsTotal: plan.EstimatedRuns},
