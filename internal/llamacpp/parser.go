...
// ParsedMetrics holds the structured metric values parsed from server output.
...
// Map returns the parsed metrics as a flat map suitable for JSON output.
...
// LogText reads the contents of a log file.
...
// EndpointKindFromSummary determines the endpoint type from a run summary's request.
...
// ClassifyHTTPError classifies an HTTP error response into a run status.
...
// ClassifyRun classifies a run's return code and output text into a status.
...
// BenchmarkInvalidReason checks whether a benchmark run produced valid results.
...
// ParseLlamaOutput parses llama-server log output for timing metrics.
...
// ParseCompletion extracts timing metrics from a completion endpoint response.
...
// ParseChatCompletion extracts timing metrics from a chat completion response.
